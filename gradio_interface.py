...
def research_interface(query, style_number):
    """
    Interface function for the Gradio app that processes research queries
    
    Args:
        query: The user's research query
        style_number: The selected content style number (1-3)
    
    Returns:
        A tuple containing the research output, fact-check report, and content draft
    """
    # Convert style number from Gradio dropdown to integer
    style_number = int(style_number.split(".")[0])
    
    # Get the content style string
    ...
    if result.get("status") == "completed":
        # Extract references for display
        references = result.get('references', [])
        references_text = "\n".join(references) if references else "No references available"
        
        # Ensure draft content includes references
        draft_with_references = result['draft_content']
        
        # If references aren't already in the draft, append them
